...
func (c *SchedulerConfiguration) Initialize() *SchedulerConfiguration {
	...
	flag.DurationVar(&c.LeaderRetryInterval, "ha.leader.retry", 2*time.Second, "How long to wait before retrying the leader election process")
	flag.DurationVar(&c.LeaderServerRetry, "ha.leader.server.retry", 2*time.Second, "How long to wait before accepting connections from clients after an error")
