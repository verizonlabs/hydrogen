...
func (e *Event) Update(updateEvent *mesos_v1_scheduler.Event_Update) {
	...
	defer func() {
		...
		_, err := e.controller.Scheduler.Acknowledge(agentID, taskID, status.GetUuid())
		if err != nil {
			e.controller.Logger.Emit(
				logging.ERROR,
	...
	task, err := e.controller.TaskManager.GetById(taskID)
	if err != nil {
	...
	task.State = state
	err = e.controller.TaskManager.Update(task)

	if err != nil {
		e.controller.Logger.Emit(logging.ERROR, "Failed to update task %s: %s", taskIdVal, err.Error())
		return
	}

	switch state {
	case mesos_v1.TaskState_TASK_FAILED:
		e.controller.Logger.Emit(logging.ERROR, "Task %s failed: %s", taskIdVal, message)
		task.Reschedule(e.controller.Revive)
	case mesos_v1.TaskState_TASK_STAGING:
		// NOP, keep task set to "launched".
		e.controller.Logger.Emit(logging.INFO, "Task %s is staging: %s", taskIdVal, message)
	case mesos_v1.TaskState_TASK_DROPPED:

		// Transient error, we should retry launching. Taskinfo is fine.
		e.controller.Logger.Emit(logging.INFO, "Task %s dropped: %s", taskIdVal, message)
		task.Reschedule(e.controller.Revive)
	case mesos_v1.TaskState_TASK_ERROR:
		e.controller.Logger.Emit(logging.ERROR, "Error with task %s: %s", taskIdVal, message)
		task.Reschedule(e.controller.Revive)
	...
	case mesos_v1.TaskState_TASK_GONE:
		...
		e.controller.Logger.Emit(logging.ERROR, "Task %s is gone: %s", taskIdVal, message)
	...
	case mesos_v1.TaskState_TASK_GONE_BY_OPERATOR:
		...
		e.controller.Logger.Emit(logging.ERROR, "Task %s gone by operator: %s", taskIdVal, message)
	...
	case mesos_v1.TaskState_TASK_KILLING:
		...
		e.controller.Logger.Emit(logging.INFO, "Killing task %s: %s", taskIdVal, message)
	case mesos_v1.TaskState_TASK_LOST:
		// Task is unknown to the master and lost. Should reschedule.
		e.controller.Logger.Emit(logging.ALARM, "Task %s was lost", taskIdVal)
		task.Reschedule(e.controller.Revive)
	...
	case mesos_v1.TaskState_TASK_STARTING:
		...
		e.controller.Logger.Emit(logging.INFO, "Task %s is starting: %s", taskIdVal, message)
	case mesos_v1.TaskState_TASK_UNKNOWN:

		// Task is unknown to the master. Should ignore.
		e.controller.Logger.Emit(logging.ALARM, "Task %s is unknown: %s", taskIdVal, message)
